...
Run `python run_pipeline.py` first. Then, in Power BI Desktop, choose
**Home > Get data > Excel workbook** and open:

```text
data\outputs\credit_risk_powerbi_dataset.xlsx
```

This one workbook is exported from PostgreSQL and contains separate Excel tables
for every analytical grain:

| Power BI query name | Excel table | PostgreSQL source | Grain |
|---|---|---|---|
| Fact Loans | `tblFact_Loans` | `credit_risk.mv_powerbi_loans` | One originated loan |
| Fact Thresholds | `tblFact_Thresholds` | `credit_risk.vw_threshold_simulator` | One selected-model test threshold |
| Fact Model Metrics | `tblFact_ModelMetrics` | `credit_risk.model_metrics` | One model/split/threshold type |
| Fact Monthly Portfolio | `tblFact_Portfolio` | `credit_risk.portfolio_metrics` | One month |
| Fact Stress | `tblFact_Stress` | `credit_risk.vw_stress_scenarios` | One scenario/sector |
| Fact Vintage | `tblFact_Vintage` | `credit_risk.vintage_analysis` | One vintage/month-on-books |
| Fact Migration | `tblFact_Migration` | `credit_risk.vw_rating_migration` | One year/sector |
| Feature Importance | `tblFeature_Importance` | `credit_risk.feature_importance` | One selected-model feature |

Select all eight tables in Navigator. Do not use `tblManifest` as an
analytical table. There is no CSV fallback in the official pipeline.
...
- Maximum: `0.99`
- Increment: `0.01`
- Default: use the `operating_threshold` generated by the current pipeline run
...
Keep recommendations in the README or portfolio narrative, not as a large text
panel on the report canvas. If context is needed, place a short methodology note
inside an information-button tooltip.
...
- Show all three required candidates: Logistic Regression, Calibrated Random
  Forest, and XGBoost.
- Populate metric cards only from the selected model generated by the current
  pipeline run. Do not retain values from an earlier run.

Limitations placed in an information-button tooltip, not a large text panel:
...
|---|---|---|---|
| Decision Threshold | What-if slicer | Decision Threshold[Decision Threshold] | Set default to the generated operating threshold |
...
Reconcile the page against `Fact Thresholds` at the selected operating threshold.
The recommended interpretation is:

> Use the validation-selected threshold as a starting point for analyst review,
> then validate review capacity, calibration, and realized costs before adopting
> a production policy.
...
1. Import the eight Excel tables from `credit_risk_powerbi_dataset.xlsx`.
...
8. Build Page 2 and validate test metrics against `Fact Model Metrics`.
9. Build Page 3 and validate the generated threshold against `Fact Thresholds`.
10. Build Page 4 and validate scenario totals against `Fact Stress`.
...
- Test rows: `11,077`.
- Score range matches the current `Fact Loans` minimum and maximum.
- Threshold confusion-matrix values match the current `Fact Thresholds` row.
- Page 1 EAD: `$164.930B`.
...
- [Microsoft: Excel workbook connector for Power Query](https://learn.microsoft.com/en-us/power-query/connectors/excel)
